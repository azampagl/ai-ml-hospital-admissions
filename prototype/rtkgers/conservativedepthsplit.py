...
class RTKGERSConservativeDepthSplit(RTKGERSCore):
    """
    """
    ...
    def grow(self, node, points):
        ...
        for f in range(len(training_points[0].features)):
            ...
            while len(indicies) > 0:
                ...
                paired_sample = stats.ttest_rel(ttest_pair_2, ttest_pair_1)
                print(original_error)
                print(error)
                print(paired_sample)
                
                if (error < original_error and paired_sample[1] < 0.05):
                    if (best_error > error):
                        best_index = i
                        best_feature = f
                        best_error = error
